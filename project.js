requestAnimationFrame([
    ...
    const webMap = new WebMap({
        portalItem: {
            id:
            "232b4d297d054b2a831a3ce629ac8495"
        }
    ...
    function toggleButton(item) {
        const layerListEl=
        document.getElementsByClassName("esri-layer-list")[0];
        let currentProp;
        const galleryEl = document.getElementsByClassName("esri-basemap-galley")[0];

        if(item == "LayerList") {
           currentProp = 
           layerListEl.style.getPropertyValue 
           ("display");
           layerListEl.style.setProperty("display", currentProp == "block" ? 
           "none" : "block");
           galleryEl.style.setProperty("display", "none");
      } else if (item == "gallery") {
          currentProp = 
          galleryEl.style.getPropertyValue ("display");

        galleryEl.style.setProperty("display", currentProp == "block"?
          "none" : "block");
